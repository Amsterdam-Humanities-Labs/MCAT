...
<Dialog bind:open title="Interrupted Run Detected" {onclose} class={className}>
  <div class="space-y-4">
    <div class="flex items-center gap-3 p-3 bg-amber-900/20 border border-amber-600/30 rounded">
    ...
    {#if run}
      <div class="grid grid-cols-2 gap-3 p-4 bg-mcat-bg rounded">
        <div>
          <span class="block text-xs text-mcat-text-muted mb-1">Run ID</span>
          <span class="font-mono text-sm">{run.runId}</span>
        </div>
        <div>
          <span class="block text-xs text-mcat-text-muted mb-1">Progress</span>
        ...
        <div>
          <span class="block text-xs text-mcat-text-muted mb-1">Completion</span>
