...
class CSVService:
    ...
    def validate_column_mapping(self, file_info: FileInfo, column_mapping: ColumnMapping) -> ValidationResult:
        ...
        # Check preserve columns exist in file
        for col in column_mapping.preserve_columns:
            if col not in file_info.columns:
                result.add_error(f"Column '{col}' not found in CSV")
        
        # If no errors, mark as valid
        if not result.errors:
            result.valid = True
            result.post_column = column_mapping.post_column
            result.preserve_columns = column_mapping.preserve_columns.copy()
